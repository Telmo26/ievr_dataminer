...
fn main() {
    ...
    let character_thread = create_character_thread(&database_root_path, &extraction_root_path, chara_requested_files, char_name_req_tx);
    ...
    let text_thread = create_text_thread(&database_root_path, &extraction_root_path, text_requested_files, char_name_req_rx);

    let _ = character_thread.join();
    let _ = text_thread.join();
}

fn create_character_thread(database_root_path: &Path, extraction_root_path: &Arc<PathBuf>, requested_files: Vec<String>, char_name_req_tx: Sender<i32>) -> JoinHandle<()> {
    ...
    thread::spawn(move || {
        populate_character_data(&extraction_path_clone, character_database, requested_files, char_name_req_tx);
    })
}

fn create_text_thread(database_root_path: &Path, extraction_root_path: &Arc<PathBuf>, requested_files: Vec<String>, char_name_req_rx: Receiver<i32>) -> JoinHandle<()> {
    ...
    let extraction_path_clone = extraction_root_path.clone();
    thread::spawn(move || {
        populate_text_data(&extraction_path_clone,text_databases, requested_files, char_name_req_rx);
    })
}
